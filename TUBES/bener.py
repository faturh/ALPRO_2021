...
def absensi(tanggal, jam_masuk, jam_keluar, id_pegawai):
    conn = create_connection()
    c = conn.cursor()
...
def admin_screen():
    ...
    if len(absensi_data) > 0:
        ...
        for data in absensi_data:
            ...
            jam_keluar = None if data[2] is None else datetime.strptime(data[2], "%H:%M:%S").time()
...
def karyawan_screen(username):
    ...
    def keluar_action():
        ...
        if karyawan_id is not None:
            ...
            if len(absensi_data) > 0 and absensi_data[-1][2] == "00:00:00":
                conn = create_connection()
                c = conn.cursor()
