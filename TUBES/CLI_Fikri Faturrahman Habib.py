...
# Fungsi unt \uk mendapatkan data absensi berdasarkan id_pegawai
...
def karyawan_screen(username):
    def absensi_action():
        ...
        cur.execute("SELECT jam_keluar FROM absensi WHERE id_pegawai = '{}' AND tanggal = '{}'".format(login_id,datetoday))
    ...
    def keluar_action():
        ...
        if len(absensi) == 0 :
          ...
          if absensi[0] == None : 
                    cur.execute("UPDATE absensi SET jam_keluar = '{}' WHERE id_pegawai = '{}' and tanggal = '{}'".format(time,login_id,datetoday))
...
# untuk rekap absen
def rekapabsen(username):
    con = create_connection()
    cur = con.cursor()
    cur.execute("SELECT id_pegawai FROM user WHERE username=%s", (username,))
    h = cur.fetchone()
    karyawan_id = h[0]
    cur.execute("SELECT tanggal,jam_masuk,jam_keluar FROM absensi WHERE id_pegawai = %s ;",(karyawan_id,))
    data = cur.fetchall()
    if len(data) > 0:
        print("Rekap absensi anda:")
        for x in range(len(data)):
            tanggal = data[x][0]
            jam_masuk = data[x][1]
            jam_keluar = data[x][2]
            print(f" Tanggal: {tanggal}, Jam Masuk: {jam_masuk}, Jam Keluar: {jam_keluar}")
    else:
        print("No absensi data available.")
