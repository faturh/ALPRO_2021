...
def admin_screen():
    ...
    if len(absensi_data) > 0:
        ...
        for data in absensi_data:
            ...
            jam_keluar = None if data[2] is None else datetime.strptime(data[2], "%H:%S:%M").time()
...
# Fungsi untuk melihat rekap absensi
def rekapabsen(username):
    conn = create_connection()
    c = conn.cursor()
    c.execute("SELECT id_pegawai, nama FROM user WHERE username=%s", (username,))
    user_data = c.fetchone()
    id_pegawai = user_data[0]
    nama = user_data[1]
    ...
    print("Rekap Absensi")
    print(f"Karyawan: {nama}")
    if len(absensi_data) > 0:
        for data in absensi_data:
            tanggal = data[0]
            jam_masuk = (datetime.min + data[1]).time()
            jam_keluar = None if data[2] is None else (datetime.min + data[2]).time()
            print(f"Tanggal: {tanggal}, Jam Masuk: {jam_masuk}, Jam Keluar: {jam_keluar}")
    else:
